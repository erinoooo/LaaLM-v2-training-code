...
OUTPUT_FILE = "laalm_v2_training_data.jsonl"
...
class FileSystem:
    ...
    def _init_home(self):
        """Initialize /home/user directory"""
    ...
    def resolve_path(self, path: str) -> str:
        ...
        if path == "..":
            if self.cwd == "/":
                return "/"
            parts = [p for p in self.cwd.strip("/").split("/") if p]
            if len(parts) <= 1:
                return "/"
            return "/" + "/".join(parts[:-1])
...
class CommandExecutor:
    ...
    def execute(self, command: str) -> Tuple[str, Optional[str]]:
        """
        Execute command and return (output, reasoning).
        reasoning is None for simple commands, string for complex ones.
        """
    ...
    def _execute_pipe(self, command: str) -> Tuple[str, str]:
        """Execute piped command with reasoning"""
        ...
        for i, part in enumerate(parts):
            ...
            if i == 0:
                ...
                if cmd_parts[0] == "grep" and len(cmd_parts) > 1:
                    ...
                    n = int(cmd_parts[1]) if len(cmd_parts) > 1 and cmd_parts[1].isdigit() else 10
    ...
    def _execute_append_redirect(self, command: str) -> Tuple[str, None]:
        ...
        # Append to file
        if self.fs.write_file(file_part, output, append=True):
            return "", None
        else:
    ...
    def _mkdir(self, args: List[str]) -> Tuple[str, None]:
        ...
        if self.fs.exists(path):
            return f"mkdir: cannot create directory '{path}': File exists", None

        if self.fs.create_dir(path):
            return "", None
        else:
            return f"mkdir: cannot create directory '{path}': No such file or directory", None
    ...
    def _rm(self, args: List[str]) -> Tuple[str, None]:
        ...
        path = args[0]
        if not self.fs.exists(path):
            return f"rm: cannot remove '{path}': No such file or directory", None

        if self.fs.is_directory(path):
            return f"rm: cannot remove '{path}': Is a directory", None
...
def generate_conversation() -> Dict:
    ...
    # Programmatic system prompt
    system_prompt = f"""INIT_STATE:
CWD={fs.cwd}
FILES=[]
ENV=USER:user,HOME:/home/user"""
    ...
    # Command weights (higher = more likely)
    command_pool = {
        "create_file": 15,
        "create_dir": 10,
        "list": 12,
    ...
    for _ in range(num_commands):
        ...
        if action == "create_file":
            ...
            if files:
                ...
                command = f"cat {file} | grep {pattern}"
            ...
            if "grep" in cmd_choice:
                command = f"grep error {fake_file}"
            else:
        ...
        if command:
            ...
            if reasoning:
                # Complex command with reasoning
                full_response = f"{reasoning}\nOUTPUT:\n{output}" if output else reasoning
